 ...
 * ✅ COOP: same-origin-allow-popups — Google OAuth popup support
 * ✅ Real-time messaging (conversations + messages tables)
 * ✅ Legacy chat system (chat_sessions + chat_messages tables)
 * ✅ Packages schema bootstrapped at startup
 * ✅ Full error details logged on boot failure
 */
...
const ensurePackagesSchema = async () => {
  ...
  // ── Admin info requests (dynamic forms) ──────────────────────────────────
  await query(`
  ...
  const indexes = [
    ...
    `CREATE INDEX IF NOT EXISTS idx_pkg_bookings_pkg     ON package_bookings(package_id)`,
    `CREATE INDEX IF NOT EXISTS idx_pkg_bookings_user    ON package_bookings(user_id)`,
    `CREATE INDEX IF NOT EXISTS idx_pkg_bookings_status  ON package_bookings(status)`,
    `CREATE INDEX IF NOT EXISTS idx_pkg_messages_pkg     ON package_messages(package_id)`,
    `CREATE INDEX IF NOT EXISTS idx_pkg_messages_sender  ON package_messages(sender_id)`,
    `CREATE INDEX IF NOT EXISTS idx_info_requests_pkg    ON admin_info_requests(package_id)`,
    `CREATE INDEX IF NOT EXISTS idx_info_requests_user   ON admin_info_requests(user_id)`,
  ];
...
// ── Headers debug ──────────────────────────────────────────────────────────────
app.get("/api/debug/headers", (_req, res) => {
  res.json({
    success: true,
...
// ── Expose io to all route handlers ───────────────────────────────────────────
app.set("io", io);

// ═══════════════════════════════════════════════════════════════════════════════
// SOCKET.IO — JWT HELPER
// ═══════════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════════
// SOCKET.IO — LEGACY CHAT DB HELPERS
// ═══════════════════════════════════════════════════════════════════════════════
...
io.use((socket, next) => {
  ...
  if (socket.data.isAdmin) {
    socket.join("admins");
    socket.join("admin-room"); // ← used by emitToAdmin() in routes/packages.js
    logger.info(`[Socket] Admin joined: ${socket.id} (adminId=${decoded?.id})`);
  }

  next();
});

// ── Typing indicator cleanup (every 15s) ──────────────────────────────────────
setInterval(async () => {
  try {
...
io.on("connection", (socket) => {
  ...
  // User room — allows emitToUser(req, userId, ...) from route handlers
...
async function initializeServer() {
  try {
    ...
    httpServer.listen(PORT, () => {
      ...
      logger.info(`  Socket.io  : msg:* + chat:* + admin:* + pkg:* events ✅`);
      logger.info(`${div}\n`);
    });
    ...
    // ✅ Full error details so Render logs show the real problem
    logger.error("❌ Server boot failed:", err.message);
    logger.error(err.stack);
    process.exit(1);
  }

  // ── Email verification (non-fatal, runs after server starts) ─────────────

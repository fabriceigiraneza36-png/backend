...
io.use((socket, next) => {
  ...
  if (socket.data.isAdmin) {
    socket.join('admins');
    logger.info(`[Socket] Admin joined room: ${socket.id} (adminId=${decoded?.id})`);
  }
...
io.on('connection', (socket) => {
  ...
  socket.on('msg:typing', async (payload = {}) => {
    ...
    try {
      if (isTyping) {
        ...
        await query(
          `DELETE FROM typing_indicators WHERE socket_id = $1`, [socket.id],
        );
      }
    } catch { /* non-fatal */ }
  ...
  socket.on('msg:admin-conversations', async (payload = {}, cb) => {
    try {
      ...
      typeof cb === 'function' && cb({ success: true, conversations: res2.rows });
    } catch (err) {
      typeof cb === 'function' && cb({ success: false, error: err.message });
    }
  });
  ...
  socket.on('chat:message', async (payload = {}, cb) => {
    try {
      ...
      const name  = String(
        payload.name  || socket.data.user?.fullName || socket.data.user?.name || 'Guest',
      );

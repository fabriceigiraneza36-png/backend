...
const pingAdminNewRequest = (booking) => {
  try {
    ...
    createNotificationInternal({
      ...
      message: `Booking ${booking?.booking_number || ""} from ${booking?.full_name || "a traveller"}${booking?.attraction_name ? ` for ${booking.attraction_name}` : ""}.`,
      actionUrl: "/bookings",
      actionLabel: "Review",
      priority: "high",
      metadata: { bookingNumber: booking?.booking_number || null, bookingType, attractionName: booking?.attraction_name || null },
    }).catch(() => {});
  } catch {
    // non-fatal
  }
...
const normalizeBookingData = (raw = {}) => {
  ...
  return {
    ...
    attraction_name: safeText(read("attraction_name", "attractionName", "attraction"), null),
  ...
  try {
    ...
    const { rows } = await query(
      `INSERT INTO bookings (
          ...
          nationality, country, destination_name, country_name, attraction_name,
